...
type GetHistoricalBySymbolAndTimestampRangeRow struct {
	Timestamp    time.Time      `json:"timestamp"`
	Symbol       string         `json:"symbol"`
...
func (q *Queries) GetHistoricalBySymbolAndTimestampRange(ctx context.Context, arg GetHistoricalBySymbolAndTimestampRangeParams) ([]GetHistoricalBySymbolAndTimestampRangeRow, error) {
	rows, err := q.db.QueryContext(ctx, getHistoricalBySymbolAndTimestampRange, arg.Symbol, arg.Timestamp, arg.Timestamp_2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetHistoricalBySymbolAndTimestampRangeRow
	for rows.Next() {
		var i GetHistoricalBySymbolAndTimestampRangeRow
		if err := rows.Scan(
			&i.Timestamp,
			&i.Symbol,
			&i.Open,
